...
class StageUtils extends BaseClass {
    ...
    private stageOnResize(): void {
        ...
        Log.getInstance().trace("是否是移动设备："+App.DeviceUtils.IsMobile);

        let ua = navigator.userAgent;
        let iPad=(ua.match(/iPad/i)!= null)? true : false;//ipad终端
        let Pad=(ua.match(/Pad/i)!= null)? true : false;//安卓平板终端
        ...
        if (a > b) {
            ...
            this.getStage().scaleMode = egret.StageScaleMode.FIXED_HEIGHT;
        }else {
            this.getStage().scaleMode = egret.StageScaleMode.FIXED_WIDTH;
        }


        // Main.stageWidth = designWidth;
        // Main.stageHeight = designHeight;
        Main.stageWidth = this.getStage().stageWidth;
        Main.stageHeight = this.getStage().stageHeight;
        Log.getInstance().trace("设备宽高："+ Main.stageWidth+"-"+Main.stageHeight);
