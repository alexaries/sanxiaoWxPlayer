 ...
 */
class Log extends BaseController1{
    private static baseController_Console:Log;
    ...
    /*日志输出*/
    //是否已经启动外部日志
    private isOpenTrace=false;
    // 是否线上版本
    private isOnLine=false;
    private env = "dev";
    public trace(obj,type=1){
     ...
     **/
    private trace1(obj, type = 1, name = "对象") {
        var self = this, num = 0;
        ...
        function myWhile(obj, name = "", str = "", _isArr = false) {
            log(str + (_isArr ? ("[") : (name + ":{")));
            for (var i in obj) {
                if (num > 400) {
                    if (type != -1)
