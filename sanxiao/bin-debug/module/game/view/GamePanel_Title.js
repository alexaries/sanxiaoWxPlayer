 ...
 */
var GamePanel_Title = (function (_super) {
    __extends(GamePanel_Title, _super);
    function GamePanel_Title() {
        ...
        _this.taskArr = [];
        _this.begin_num = 3;
        _this.game_is_over = false;
        _this.time_num = 0;
        //游戏结果 1是成功 0是失败
        _this.game_over = 0;
        _this.isComplete_over = false;
        return _this;
    }
    GamePanel_Title.prototype.addStepNum = function (num) {
        if (this.isTimeLimit)
            return;
        this.set_step(1, num);
    };
    GamePanel_Title.prototype.CutStepNum = function (num) {
        if (this.isTimeLimit)
            return;
        this.set_step(0, num);
    };
     ...
     * @param num
     */
    GamePanel_Title.prototype.addTimeNum = function (num) {
        if (!this.isTimeLimit)
            return;
        this.set_Time(1, num);
    };
     ...
     * @param num
     */
    GamePanel_Title.prototype.cutTimeNum = function (num) {
        if (!this.isTimeLimit)
            return;
        this.set_Time(0, num);
    };
    GamePanel_Title.prototype.AddScore = function (num) {
        this.set_score(1, num);
    };
    GamePanel_Title.prototype.AddTime = function (num) {
        if (!this.isTimeLimit)
            return;
        this.set_Time(1, num);
    };
    GamePanel_Title.prototype.ItemFlyToGoal = function (item, id) {
        this.seekitemFunction(item); //判断功能
        this.setRect(item, id);
    };
    //游戏主逻辑请求判定游戏是否结束
    GamePanel_Title.prototype.AskForIsGameOver = function (num) {
        if (this.downOver()) {
     ...
     * 初始化属性
     */
    GamePanel_Title.prototype.init = function () {
    ...
    //开始倒计时
    GamePanel_Title.prototype.begin_times = function () {
        if (this.isTimeLimit) {
            this.GameOver1(true);
            if (this.begin_num <= 0) {
    ...
    //初始化布局
    GamePanel_Title.prototype.initView = function () {
        ...
        for (var i = 0; i < 3; i++) {
            var star1 = this.getStar(i);
            var temp = this.score_arr[i] / this.full_all * 400;
     ...
     *  更新时间
     */
    GamePanel_Title.prototype.update_time = function () {
        this.set_Time(0, 1);
    };
    /**
     * 游戏结算
     */
    GamePanel_Title.prototype.doEndingTime = function () {
        var step_temp = (this.isTimeLimit) ? Math.floor(this.timeLimitLength / 5) : this.stepNum;
        //剩余步数换成得分
        this.set_score(1, step_temp * ScoreType.ST_STEPSCORE);
    };
    GamePanel_Title.prototype.update_step = function () {
        this.step_text.text = this.getFontText();
        if (TG_Game.currentState == 2) {
            return;
        }
        else {
            if (this.downOver()) {
    ...
    GamePanel_Title.prototype.getFontText = function () {
        if (this.isTimeLimit) {
    ...
    GamePanel_Title.prototype.resize = function () {
        this.gatherSp.y = 50 + (GamePanel_BgSp.getInstance().topBg.height - 50) / 2 - this.gatherSp.height / 2;
        this.roleSp.y = GamePanel_BgSp.getInstance().topBg.y + GamePanel_BgSp.getInstance().topBg.height - this.roleSp.height1 - 30;
    };
    //添加任务系统
    GamePanel_Title.prototype.addTesk = function () {
        //初始化任务
        for (var i = 0; i < this.taskArr.length; i++) {
            var obj = this.taskArr[i];
            var obj11 = TG_MapData.getInstance().mapConfigData[obj.Target];
            var str = LoadNetworkImageUtils.getResNameByLayerId(obj11.layerid); //obj11.image+"_png";
            var item = new GatherItem(str, obj.Num, this.ruleType, obj.Target);
            var child_num = this.gatherSp.numChildren;
            //行
            var r = child_num % 3;
            //列
            var c = Math.floor(child_num / 3);
    ...
    //判断块的功能 加属性
    GamePanel_Title.prototype.seekitemFunction = function (items) {
        if (items.isFunction) {
        ...
        if (items.isPropBox) {
            //是道具宝箱
            //宝箱id
            var propId = items.propId;
            var propNum = items.propNum;
            App.MessageCenter.dispatch(Msg.Event.UseProp, propId, 1, propNum); //派发更新道具数量事件
        }
    };
    //设置消除格子信息
    GamePanel_Title.prototype.setRect = function (items, id) {
        try {
            var _x = 0;
            var _y = 0;
            var pos = items.SitePos;
            if (pos) {
                var pos1 = items.getPosByRowCol(pos.Y, pos.X);
                _x = pos1.x;
                _y = pos1.y;
            }
            // let texture:string = (items.getChildAt(0) as TG_Object).textureName;
            var _id = id; //items["BlockId"];
            var obj12 = TG_MapData.getInstance().mapConfigData[_id];
            var str = LoadNetworkImageUtils.getResNameByLayerId(obj12.layerid); //obj12.image + "_png";
            var item = null;
            for (var i = 0; i < this.taskItemArr.length; i++) {
                var item1 = this.taskItemArr[i];
                if (item1.id == _id) {
                    item = item1;
                    break;
                }
            ...
            if (item != null) {
                var btm = ObjectPool.pop("egret.Bitmap");
    ...
    GamePanel_Title.prototype.getStar = function (num) {
        var sp = new egret.Sprite();
        var bm2 = new egret.Bitmap();
        bm2.texture = RES.getRes("scoreLine_png");
        sp.addChild(bm2);
        var bm3 = new egret.Bitmap();
     ...
     * @param num
     */
    GamePanel_Title.prototype.set_step = function (type, _step) {
        if (this.isTimeLimit)
            return;
        var step = Math.abs(_step);
        if (type == 1)
            this.stepNum += step;
        else
            this.stepNum -= step;
     ...
     * @param num
     */
    GamePanel_Title.prototype.set_Time = function (type, num) {
     ...
     * @num 需要加的分数
     * */
    GamePanel_Title.prototype.set_score = function (type, num) {
        var score = Math.abs(num);
        if (type == 1)
        ...
        for (var i = 0; i < 3; i++) {
            if (this.score_num >= this.score_arr[i] && this.score_boo[i] == false) {
                var star = void 0;
                star = this.star_arr[i];
                var sp = star.getChildByName("star_" + i);
                if (sp) {
                    sp.texture = RES.getRes("scoreStar0_png");
                }
            }
        }
        var tmp = this.score_num / this.full_all * 400;
        if (tmp > 400)
            tmp = 400;
    ...
    //判断游戏结果
    GamePanel_Title.prototype.downOver = function () {
    ...
    GamePanel_Title.prototype.GameOver = function () {
        if (this.game_over == 1) {
    ...
    GamePanel_Title.prototype.GameOver1 = function (bool) {
    ...
    return GamePanel_Title;
}(BaseClassSprite));
__reflect(GamePanel_Title.prototype, "GamePanel_Title");
/**
 *  收集items
 */
var GatherItem = (function (_super) {
     ...
     */
    function GatherItem(textureName, _num, _ruleType, _id) {
        ...
        _this.btm.width = TG_Item.getInstance().itemWidth;
        _this.btm.height = TG_Item.getInstance().itemWidth;
        ...
        _this.num_tex.y = _this.btm.y + _this.btm.height + 5;
        _this.addChild(_this.num_tex);
        return _this;
    }
    GatherItem.prototype.updateRectNum = function (_num) {
        if (this.ruleType == 0 && this.isComplete == false) {
            ...
            if (this.num <= 0) {
                ...
                if (TG_Stage.SingelModel) {
                    //完成
                    var b = TG_Object.Create("duihao_png");
...
__reflect(GatherItem.prototype, "GatherItem");
//# sourceMappingURL=GamePanel_Title.js.map